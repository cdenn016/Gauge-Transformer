...
\centering
\small
\caption{Model capacity ablation. All models use 1 layer, $\kappa_\beta=1$, 20k training steps, context length 128.}
...
SO(20) & 20 & 26.4M & 369 & 318 & 158$\times$ \\
...
Performance improves with gauge group dimension up to SO(50), achieving optimal test PPL of 299. Beyond SO(50), performance degrades despite increased parameters, suggesting optimization difficulties at larger scales or insufficient training steps for larger models.
...
\centering
\small
\caption{VFE iteration ablation. SO(50), $K=50$, 1 layer, 20k steps.}
\label{tab:ablation_vfe}
\begin{tabular}{rrrr}
\toprule
\textbf{VFE Iterations} & \textbf{Val PPL} & \textbf{Test PPL} & \textbf{vs Random} \\
\midrule
1 & 274 & 299 & 168$\times$ \\
\textbf{5} & \textbf{265} & \textbf{270} & \textbf{186$\times$} \\
\bottomrule
\end{tabular}
\end{table}

Increasing VFE iterations from 1 to 5 yields substantial improvement (test PPL 299 $\to$ 270), demonstrating that deeper inference within the forward pass benefits performance. This is our best result overall.
...
\centering
\small
\caption{Gauge group structure ablation comparing SO(3) with multiple irreps vs SO(50) fundamental.}
\label{tab:ablation_irrep}
\begin{tabular}{llrrrr}
\toprule
\textbf{Group} & \textbf{Irrep Structure} & \textbf{K} & \textbf{Params} & \textbf{Test PPL} \\
\midrule
SO(3) & $\ell=1,4,5,6,7$ (5 irreps) & 51 & 7.8M & 291 \\
SO(3) & fundamental ($\ell=1$) & 60 & 9.2M & 327 \\
...
\subsection{Temperature and Step Size}

Table~\ref{tab:ablation_hyperparams} shows sensitivity to attention temperature $\kappa_\beta$ and learning rate.

\begin{table}[h]
\centering
\small
\caption{Hyperparameter sensitivity. SO(50), $K=50$, 1 layer, context 128.}
\label{tab:ablation_hyperparams}
\begin{tabular}{llrrr}
\toprule
\textbf{Ablation} & \textbf{Setting} & \textbf{Steps} & \textbf{Val PPL} & \textbf{Test PPL} \\
\midrule
\multicolumn{5}{l}{\textit{Temperature $\kappa_\beta$}} \\
\quad Baseline & $\kappa_\beta = 1.0$ & 20k & 274 & 299 \\
\quad Higher temp & $\kappa_\beta = 1.75$ & 20k & 301 & 303 \\
\midrule
\multicolumn{5}{l}{\textit{Learning rate (SO(20), $K=20$)}} \\
\quad Baseline & $\eta = 0.01$ & 20k & 369 & 318 \\
\quad Low LR & $\eta = 0.001$ & 40k & 559 & 563 \\
\quad Undertrained & $\eta = 0.01$ & 2k & 739 & 768 \\
\bottomrule
\end{tabular}
\end{table}

The model is relatively robust to temperature ($\kappa_\beta = 1.0$ vs 1.75 yields only 4 PPL difference), but sensitive to learning rate. Lower learning rates with longer training underperform, suggesting the natural gradient benefits from larger step sizes.

\subsection{Key Findings}

\begin{enumerate}
\item \textbf{Optimal capacity:} SO(50) with $K=50$ provides the best single-irrep performance; larger groups show diminishing returns.

\item \textbf{Multiple VFE iterations:} Increasing from 1 to 5 iterations improves test PPL from 299 to 270 (our best result).

\item \textbf{Irrep efficiency:} SO(3) with multiple irreps ($\ell=1,4,5,6,7$) achieves PPL 291 with only 7.8M params, outperforming 69.1M param SO(50). This suggests exploring richer irrep structures is a promising direction.

\item \textbf{Robustness:} Temperature $\kappa_\beta$ and learning rate have modest effects within reasonable ranges.
\end{enumerate}
