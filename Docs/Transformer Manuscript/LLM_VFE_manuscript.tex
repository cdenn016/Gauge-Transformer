...
In the symmetric phase where all agents are aligned via parallel transport ($\mu_i = \Omega_{ij}\mu_j$ for all connected pairs), the consistency term vanishes. Combined with the prior term, the vacuum ground state is:
\begin{equation}
\mu^* = 0
\end{equation}

However, for non-zero prior variance, there exists a family of degenerate minima related by gauge transformations: if $\{\mu_i^*\}$ is a minimum, so is $\{g_i \mu_i^*\}$ for any consistent choice of $g_i \in G$. This is the gauge-invariant ground state shown in Figure~\ref{fig:vacuum}.
