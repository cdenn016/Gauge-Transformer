...
The emergence of semantic structure in gauge frames is not accidental but follows from a precise mathematical relationship between gauge-theoretic and standard transformer attention. In companion theoretical work (under review), we derive that standard transformer attention emerges from the gauge framework under three successive limits: (1) isotropic covariances ($\Sigma_i = \sigma^2 I$), (2) flat bundle ($\Omega_{ij} = \Omega$ for all pairs), and (3) learned projections.

Under these limits, the attention compatibility score reduces to:
\begin{equation}
s_{ij} = D_{\mathrm{KL}}(q_i \| \Omega q_j) \to \frac{1}{2}\|\mu_i - \Omega\mu_j\|^2 \to \mu_i^\top \Omega \mu_j + O(1),
\end{equation}
where the final form emerges after softmax cancellation of query-independent terms (with scaling absorbed into temperature). Defining learned projection matrices $A, B \in \mathbb{R}^{d \times d_k}$ such that
\begin{equation}
AB^\top \propto \Omega,
\label{eq:ab_omega}
\end{equation}
we recover the standard dot-product attention: $Q_i K_j^\top = \mu_i^\top A B^\top \mu_j \propto \mu_i^\top \Omega \mu_j$, which yields the familiar $\mathrm{softmax}(QK^\top/\sqrt{d_k})V$ after temperature scaling.
...
\paragraph{Limit 1: Isotropic covariances ($\Sigma_i = \sigma^2 I$).}
With shared isotropic covariances, the KL divergence simplifies to a squared Euclidean distance (up to scaling):
\begin{equation}
D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j) \propto \|\mu_i - \Omega_{ij}\mu_j\|^2.
\end{equation}
...
\begin{enumerate}
    \item Covariances are isotropic (scaling absorbed into temperature)
...
The key insight is Equation~\eqref{eq:ab_omega}: the learned $W_Q W_K^\top$ in standard transformers plays the role of $\Omega$---the gauge transport operator (up to scaling). This explains why gauge frames encode semantic structure: they are learning the geometric relationships that standard transformers encode in learned projections.
