...
The gauge-theoretic framework~\citep{Dennis2025} treats each token as an autonomous agent maintaining probabilistic beliefs. Agent $i$ has beliefs $q_i(x)$, priors $p_i(x)$ (probability distributions over latent semantic content), and a gauge frame $\phi_i \in \mathfrak{g}$ encoding its local coordinate system. Communication between agents occurs via parallel transport $\Omega_{ij} = \exp(\phi_i) \cdot \exp(-\phi_j)$, which aligns agent $j$'s belief into agent $i$'s reference frame. Attention weights emerge from KL divergence measuring belief alignment after transport:
%
\begin{equation}
