...
\begin{table}[t]
\centering
\caption{Performance on WikiText-103 token-level modeling. All models trained for 200,000 steps with batch size 3 and context length 128. Learning rates: standard transformer uses $\eta = 3 \times 10^{-4}$; gauge VFE uses $\eta = 0.01$.}
\label{tab:main_results}
\begin{tabular}{lcccccc}
\toprule
\textbf{Architecture} & \textbf{Layers} & \textbf{Params} & \textbf{Val PPL} $\downarrow$ & \textbf{Test PPL} $\downarrow$ & \textbf{Loss} & \textbf{vs Random} \\
\midrule
Random baseline & --- & --- & 50,257 & 50,257 & 10.82 & 1$\times$ \\
\midrule
...
The gauge-theoretic framework presented in this paper specializes to SO($N$) gauge groups and multivariate Gaussian belief distributions. Here we briefly outline the general formulation, which encompasses arbitrary compact Lie groups and exponential family distributions. Full derivations are deferred to future work.

\subsection{Arbitrary Gauge Groups}

The framework extends to any compact Lie group $G$ with finite-dimensional unitary representations $\rho: G \to \mathrm{GL}(V)$. The key structures generalize as:

\begin{itemize}
    \item \textbf{Gauge frames:} $\phi_i \in \mathfrak{g}$ (the Lie algebra of $G$)
    \item \textbf{Parallel transport:} $\Omega_{ij} = \rho(\exp(\phi_i))\rho(\exp(-\phi_j)) \in \mathrm{GL}(V)$
    \item \textbf{Multi-head structure:} Determined by the irreducible decomposition $\rho = \bigoplus_k n_k \rho_k$
\end{itemize}

For non-Abelian groups (SU($N$), Sp($N$), exceptional groups), the irrep decomposition provides richer multi-head structures with different transformation properties. Abelian groups (U(1)$^N$) recover diagonal attention patterns.

\subsection{Exponential Family Beliefs}

Gaussian distributions are a special case of exponential families. The general formulation uses:

\begin{equation}
q_i(\xi) = h(\xi) \exp\left[\eta_i^\top T(\xi) - A(\eta_i)\right]
\end{equation}

where $\eta_i \in \mathcal{H}$ are natural parameters, $T(\xi)$ are sufficient statistics, and $A(\eta)$ is the log-partition function. The KL divergence becomes the Bregman divergence:

\begin{equation}
D_{\mathrm{KL}}(q_i \| q_j) = D_A(\eta_j \| \eta_i) = A(\eta_j) - A(\eta_i) - \nabla A(\eta_i)^\top(\eta_j - \eta_i)
\end{equation}

Parallel transport acts on natural parameters via the representation $\rho$: $\eta_i \to \Omega_{ij}\eta_i$. The Fisher-Rao metric $g_{ab} = \partial_a \partial_b A(\eta)$ provides the natural gradient structure.

\subsection{Mixture Distributions}

For discrete latent structure, beliefs can be mixtures:

\begin{equation}
q_i(\xi) = \sum_{k=1}^K \pi_i^{(k)} q_i^{(k)}(\xi)
\end{equation}

where $\pi_i \in \Delta^{K-1}$ are mixing weights. The gauge group acts on both the component parameters and potentially the mixture weights. This enables modeling of multi-modal beliefs and compositional semantics.

\subsection{Scope of Current Work}

This paper restricts to SO($N$) groups and Gaussians for several reasons: (1) closed-form KL divergences enable efficient computation, (2) the fundamental representation provides interpretable attention heads, and (3) the orthogonal group preserves Euclidean structure natural for embedding spaces. The generalizations above indicate that the framework is not limited to these choices, with richer group structures potentially capturing more complex linguistic phenomena.
