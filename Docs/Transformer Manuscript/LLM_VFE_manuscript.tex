...
All models implemented in Python 3.12 using PyTorch 2.x with CUDA support. Standard transformer baselines use learned positional embeddings; the gauge VFE uses no explicit positional encoding, with positional information emerging implicitly through the attention mechanism and gauge frame structure. Natural gradient computations use diagonal covariance approximation for efficiency, and transport operators are computed via PyTorch's \texttt{matrix\_exp}. Training performed on a single NVIDIA RTX 5090 GPU (32GB). Code available at \url{https://github.com/cdenn016/Gauge-Transformer}.
