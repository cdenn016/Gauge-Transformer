...
\begin{itemize}
  \item Learned token embeddings: $V \times d$ matrix, $d=320$
  \item Dot-product attention: $\text{softmax}(QK^\top/\sqrt{d})V$ with learned projections
  \item Feed-forward MLP: 2-layer network with GELU, hidden dimension 1280
  \item Optimization: Adam~\citep{kingma2014adam} with learning rate $\eta = 3 \times 10^{-4}$
  \item Layers: 6
  \item Total parameters: $\sim$24M (matched to gauge VFE)
\end{itemize}
...
\begin{itemize}
  ...
  \item Optimization: Adam with learning rate $\eta = 3 \times 10^{-4}$
  \item Layers: 6
...
\begin{itemize}
  ...
  \item Batch size: 3 sequences (limited by GPU memory for $K=100$ gauge VFE on single RTX 5090)
  ...
  \item Random seed: 6 (for reproducibility)
\end{itemize}
...
\begin{tabular}{lccccc}
\toprule
\textbf{Architecture} & \textbf{Dim} & \textbf{Layers} & \textbf{Heads} & \textbf{FFN} & \textbf{Params} \\
\midrule
Standard (param-match) & 320 & 6 & 8 & MLP (1280) & 23.5M \\
Standard (embed-match) & 100 & 6 & 4 & MLP (400) & 5.76M \\
Gauge VFE (SO(20)) & 100 & 1 & 5 & VFE dynamic & 24.6M \\
\bottomrule
\end{tabular}
\end{table}
...
The emergence of semantic structure in gauge frames is not accidental but follows from a precise mathematical relationship between gauge-theoretic and standard transformer attention. In companion theoretical work (under review), we derive that standard transformer attention emerges from the gauge framework under three successive limits: (1) deterministic beliefs ($\Sigma_i \to 0$), (2) flat bundle ($\Omega_{ij} = \Omega$ for all pairs), and (3) learned projections.

Under these limits, the attention compatibility score reduces to:
\begin{equation}
s_{ij} = D_{\mathrm{KL}}(q_i \| \Omega q_j) \to \frac{1}{2\sigma^2}\|\mu_i - \Omega\mu_j\|^2 \to \frac{1}{\sigma^2}\mu_i^\top \Omega \mu_j + O(1),
\end{equation}
where the final form emerges after softmax cancellation of query-independent terms. Defining learned projection matrices $A, B \in \mathbb{R}^{d \times d_k}$ such that
\begin{equation}
AB^\top = \frac{1}{\sigma^2} \Omega,
\label{eq:ab_omega}
\end{equation}
we recover the standard dot-product attention: $Q_i K_j^\top = \mu_i^\top A B^\top \mu_j = \frac{1}{\sigma^2}\mu_i^\top \Omega \mu_j$, which yields the familiar $\mathrm{softmax}(QK^\top/\sqrt{d_k})V$ after temperature scaling.
...
With isotropic covariances $\Sigma_i = \Sigma_j = \sigma^2 I$, the KL divergence simplifies to a squared Euclidean distance:
\begin{equation}
D_{\mathrm{KL}}(q_i \| \Omega_{ij} q_j) \to \frac{1}{2\sigma^2}\|\mu_i - \Omega_{ij}\mu_j\|^2.
\end{equation}
...
\begin{equation}
s_{ij} = \frac{1}{2\sigma^2}\|\mu_i - \Omega\mu_j\|^2 = \frac{1}{2\sigma^2}\|\mu_i\|^2 + \frac{1}{2\sigma^2}\|\Omega\mu_j\|^2 - \frac{1}{\sigma^2}\mu_i^\top \Omega \mu_j.
\end{equation}

Under softmax normalization:
\begin{itemize}
    \item The query-dependent term $\frac{1}{2\sigma^2}\|\mu_i\|^2$ cancels (independent of $j$)
    \item The key-dependent term $\frac{1}{2\sigma^2}\|\Omega\mu_j\|^2$ approximately cancels due to high-dimensional concentration or layer normalization
\end{itemize}

The dominant contribution is the bilinear term $\frac{1}{\sigma^2}\mu_i^\top \Omega \mu_j$.
...
Such factorizations always exist (e.g., via SVD). Defining queries and keys:
\begin{equation}
...
Normalizing by $\sqrt{d_k}$ to produce $O(1)$ pre-softmax logits identifies the natural temperature:
\begin{equation}
\tau = \sigma^2\sqrt{d_k},
\end{equation}
encoding both belief uncertainty ($\sigma^2$) and embedding dimension ($d_k$).
...
This derivation establishes that standard transformer attention is the \emph{degenerate limit} of gauge-theoretic attention when:
\begin{enumerate}
    \item Beliefs collapse to point estimates (no uncertainty)
...
The key insight is Equation~\eqref{eq:ab_omega}: the learned $W_Q W_K^\top$ in standard transformers plays the role of $\Omega/\sigma^2$---the (scaled) gauge transport operator. This explains why gauge frames encode semantic structure: they are learning the geometric relationships that standard transformers encode in learned projections.
