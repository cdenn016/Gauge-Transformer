...
\begin{itemize}[leftmargin=*]
\item \textbf{Prior mass} $\Sigma_{p,i}^{-1}$: Resistance to change from prior beliefs. High prior precision (low $\Sigma_p$) creates high inertia.
\item \textbf{Observation mass} $\Sigma_{o,i}^{-1}$: Grounding through sensory evidence. Precise observations anchor beliefs against social pressure.
\item \textbf{Incoming social mass} $\sum_j \beta_{ij} \Omega_{ij} \Sigma_{q,j}^{-1} \Omega_{ij}^T$: Inertia from attending to confident neighbors. Being pulled toward precise sources increases resistance.
\item \textbf{Outgoing social mass} $\sum_j \beta_{ji} \Sigma_{q,i}^{-1}$: Recoil from exerting influence on others. Agents with many followers (high $\sum_j \beta_{ji}$) acquire epistemic rigidity---the ``burden of leadership.''
\end{itemize}
...
\item \textbf{Strong sensory grounding} (low $\Sigma_o$) → large $M$ → anchored beliefs
\item \textbf{Many followers} (large $\sum_j \beta_{ji}$) → large outgoing social mass → small updates
\item \textbf{Attending to confident neighbors} (large $\sum_j \beta_{ij}\tilde{\Sigma}_{q,j}^{-1}$) → large incoming social mass → small updates
\end{itemize}
...
The novel insight is the \emph{outgoing social mass} term $\sum_j \beta_{ji} \Sigma_{q,i}^{-1}$ in the mass matrix---representing recoil from influencing others. This predicts:
\begin{enumerate}
\item \textbf{Audience-dependent bias:} The same individual should exhibit stronger confirmation bias when they have many followers (e.g., public intellectuals vs. private citizens), because $\sum_j \beta_{ji}$ increases with audience size.
\item \textbf{Self-precision-weighted:} The effect scales with agent $i$'s own precision $\Sigma_{q,i}^{-1}$, so confident leaders experience more rigidity than uncertain ones with the same follower count.
\item \textbf{Dynamic:} As attention structure changes (e.g., going viral), confirmation bias should increase in real-time via the $\beta_{ji}$ terms.
\end{enumerate}
