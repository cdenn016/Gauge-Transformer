...
Each agent $i$ at sequence position $\text{pos}_i$ has a gauge frame $\phi_i \in \mathfrak{so}(N)$, which decomposes into $\frac{N(N-1)}{2}$ scalar components. For SO(3), this gives three components:
...
Standard (embed-matched) & 6 & $\sim$6M & XXX & X.XX & XXX$\times$ \\
\midrule
...
Standard (embed-matched) & 100 & 6 & 4 & Learned (400) & $\sim$6M \\
...
The comparison with standard transformers reveals [TO BE COMPLETED WITH EXPERIMENTAL RESULTS]:

\begin{itemize}
  \item \textbf{Parameter-matched comparison:} [Results pending]
  \item \textbf{Embedding-matched comparison:} [Results pending]
\end{itemize}
