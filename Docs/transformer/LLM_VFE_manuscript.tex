...
We implement and validate gauge-theoretic transformers using natural gradient descent on statistical manifolds with multi-head attention defined by Lie group structure. On token-level language modeling (WikiText-103, vocabulary 50,257), our single-layer SO(20) gauge architecture achieves perplexity XXX using pure variational inference---without MLPs, activation functions, or learned weight matrices. We compare against standard transformers under matched conditions: (1) parameter-matched ($\sim$25M parameters) and (2) embedding-matched (dimension 100), both trained for 200k steps with identical learning rates and batch sizes.

This proof-of-principle establishes that gauge-theoretic attention is not merely mathematically feasible but operationally viable at production scale. Our architecture achieves XXX$\times$ improvement over random chance through geometric structure alone, validating that transformer-like behavior emerges from gauge-theoretic principles. While standard transformers with equivalent resources achieve [better/comparable] absolute performance, our results demonstrate that neural networks are not strictly necessary for attention-based sequence modeling. We conclude that neural architectures are, in this view, computational approximations to a deeper information gauge geometry that transcends the field of machine learning.
\end{abstract}
...
This provides 5 copies of the 20-dimensional fundamental representation. The Lie algebra $\mathfrak{so}(20)$ has dimension $\frac{20 \times 19}{2} = 190$, though we use 20 rotation generators corresponding to independent rotation planes as attention heads.
...
\begin{itemize}
  ...
  \item Gauge group: SO(20) with 20 attention heads from Lie algebra generators
...
The number of attention heads is determined by the Lie algebra dimension: for $G = \text{SO}(20)$, we have $H = \dim(\mathfrak{so}(20)) = 190$ generators, though in practice we use $H = 20$ independent rotation planes as attention heads. Each head $h$ computes transport via:
\begin{equation}
...
\begin{itemize}
\item \textbf{20 attention heads}: Each corresponding to a rotation generator in SO(20)
\item \textbf{Geometric interpretability}: Features within each 20-dimensional block transform equivariantly
...
Table~\ref{tab:main_results} presents our primary results on WikiText-103 token-level language modeling. We compare the gauge VFE architecture against standard transformers under two controlled conditions: parameter-matched ($\sim$25M parameters) and embedding-matched (dimension 100).

\begin{table}[t]
\centering
\caption{Validation set performance on WikiText-103 token-level modeling. All models trained for 200,000 steps with batch size 3 and context length 128. Learning rates were tuned per architecture: standard transformers use lr=3e-4 (parameter-matched) or lr=0.01 (embedding-matched); gauge VFE uses lr=0.01.}
\label{tab:main_results}
\begin{tabular}{lcccccc}
\toprule
\textbf{Architecture} & \textbf{Layers} & \textbf{Params} & \textbf{LR} & \textbf{PPL} $\downarrow$ & \textbf{Loss} & \textbf{vs Random} \\
\midrule
Random baseline & --- & --- & --- & 50,257 & 10.82 & 1$\times$ \\
\midrule
Standard (param-matched) & 6 & 25M & 3e-4 & 146 & 4.98 & 344$\times$ \\
Standard (embed-matched) & 6 & 5.76M & 0.01 & 447 & 6.10 & 112$\times$ \\
\midrule
Gauge VFE (SO(20)) & 1 & 24.6M & 0.01 & 230 & 5.44 & 218$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Standard (param-matched) & 256 & 6 & 8 & Learned (1024) & $\sim$25M \\
Standard (embed-matched) & 100 & 6 & 4 & Learned (400) & 5.76M \\
Gauge VFE (SO(20)) & 100 & 1 & 20 & VFE\_dynamic & 24.6M \\
\bottomrule
\end{tabular}
\end{table}
...
The comparison with standard transformers reveals several findings:

\begin{itemize}
  \item \textbf{Parameter-matched comparison:} With comparable parameter counts ($\sim$25M), the optimally-tuned standard transformer (lr=3e-4) achieves better perplexity than gauge VFE (146 vs 230). This is expected: standard transformers benefit from decades of architectural refinement and dedicated hardware optimization. The gauge VFE achieves 63\% of the standard's performance using only geometric structure.

  \item \textbf{Embedding-matched comparison:} With identical embedding dimension (100), the gauge VFE achieves 1.94$\times$ better perplexity (230 vs 447) despite using only 1 layer compared to 6 layers in the standard transformer. The standard transformer's 5.76M parameters are distributed across learned attention projections ($W_Q, W_K, W_V$) and feed-forward networks, while the gauge VFE's 24.6M parameters reside entirely in variational embeddings with geometric structure.

  \item \textbf{Learning rate robustness:} The gauge VFE trains stably at lr=0.01, while the parameter-matched standard transformer fails catastrophically at this learning rate (PPL 963 vs 146 with lr=3e-4). This robustness stems from the natural gradient structure on the statistical manifold, which provides better conditioning than standard gradient descent.
\end{itemize}

Notably, the gauge VFE uses a \emph{single geometric layer} compared to 6 layers in the standard baselines. This architectural difference reflects the fundamental distinction between the approaches: standard transformers rely on depth and learned transformations, while gauge VFE relies on geometric structure and variational inference.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../230_K=100_N=128_so20_200k_3batch_ffn_VFE_dynamic/publication_outputs/VFE_dynamic_20260110_153830/figures/training_curves.png}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        % \includegraphics[width=\textwidth]{figures/standard_training_curves.png}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../230_K=100_N=128_so20_200k_3batch_ffn_VFE_dynamic/publication_outputs/VFE_dynamic_20260110_153830/figures/train_val_gap.png}
        \caption{Gauge VFE (SO(20))}
        \label{fig:gap_vfe}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        % \includegraphics[width=\textwidth]{figures/standard_train_val_gap.png}
...
\begin{enumerate}
  ...
  \item \textbf{Geometric multi-head attention:} The number of attention heads emerges from gauge group structure (SO(20) $\rightarrow$ 20 heads) rather than being an arbitrary hyperparameter.
