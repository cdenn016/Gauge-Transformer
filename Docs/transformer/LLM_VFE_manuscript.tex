...
\begin{table}[t]
\centering
\caption{Validation set performance on WikiText-103 token-level modeling. All models trained for 200,000 steps with learning rate 0.01, batch size 3, and context length 128. The gauge VFE achieves 218$\times$ improvement over random baseline through geometric inference alone, without neural network components.}
...
\midrule
Standard (param-matched) & 6 & $\sim$25M & XXX & X.XX & XXX$\times$ \\
Standard (embed-matched) & 6 & 5.76M & 447 & 6.10 & 112$\times$ \\
\midrule
...
The comparison with standard transformers reveals striking differences:

\begin{itemize}
  \item \textbf{Parameter-matched comparison:} [Results pending]
